...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $get_detail_sql = "SELECT item_id FROM `order_detail` WHERE order_id='$id' AND deleted_at IS NULL";
    $get_detail_res = $mysqli->query($get_detail_sql);
    $detail_id = [];
    while ($get_detail_row = $get_detail_res->fetch_assoc()) {
        array_push($detail_id, $get_detail_row['item_id']);
    }

    $ids = implode(',', $detail_id);

    $items_sql = "SELECT id, name, category_id, image, price, code_no FROM `item` WHERE id IN ($ids) AND status = '$enable_status' AND deleted_at is NULL";
    $item_result = $mysqli->query($items_sql);
    ...
    while ($item_row = $item_result->fetch_assoc()) {
        ...
        // Fetch discount based on item_id
