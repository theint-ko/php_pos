...
if($shift_check_rows>0){
    $url=$cp_base_url . "item_list.php?err=shift";
...
$delete_sql= "UPDATE `item` SET deleted_at = '$today_dt' ,deleted_by = '$user_id' WHERE id = '$id'";
...
<script>
    Swal.fire({
        ...
        if (result.isConfirmed) {
                    ...
                    if (data.status === 'success') {
                        Swal.fire({
                            ...
                            window.location.href =
                                '<?php echo $cp_base_url; ?>item_list.php?msg=delete';
                        });
                    } else {
                        Swal.fire({
                            title: "Error!",
            ...
            window.location.href = '<?php echo $cp_base_url; ?>item_list.php';
        }
    });
</script>
